...
template<class T>
void search_node(node<T> *r, T d, node<T> *&g)
{
    for(g=r; g&&g->data!=d;)
    {
...
void select_index(string query)
{
    ...
    if(tipo_dato=="int")
    {
        Deserialize_int(avl1.root, txt);
        int key = stoi(process[3]);
        //search_node(avl1.root, key, tmp1);
    }
    else
    {
        string key;
        key = process[3];
        Deserialize_int(avl2.root, txt);
        //search_node(avl2.root, key, tmp2);
    }
    cout<<"hasta qui"<<endl;
    avl1.inorder(avl1.root);
}
